...
* **依赖装进系统 python `/usr/local/lib/python3.11/site-packages`**，不要建 venv：
  `/`（系统盘）随镜像走，`/root/private_data` 是平台单独分配的持久盘（可调容量，
  当前 150GB），**独立、不随镜像、也不会被清除**。
  体积限制是 **docker 单层 ≤ 15GB**，不是镜像总大小 —— 所以按"依赖 / 代码 / 权重"
  拆层就不会撞上限。
...
* ★ 【实测】**容器没有直连出网，全部走内网 HTTP 代理**，而这套 proxy 变量**只存在于
  ...
  在 Jupyter 终端里却一切正常。诊断方法：
  `tr ' ' '
' < /proc/$(pgrep -f jupyter-lab | head -1)/environ | grep -i proxy`。
  修复脚本 `scnet_env.sh`（DTK env + 代理，优先从 jupyter 进程动态读取凭据）见 §2.6。
* 【实测】**重启实例会把 `/` 回退到基础镜像**——装在系统 python 里的依赖全部消失
  （除非先"关机时保存开发环境"）。`/root/private_data` 下的东西不受影响。

## 2.6 `scnet_env.sh`（每个 SSH 会话都要 source）

容器缺两样东西，都不是"没适配"，是环境没接上：DTK 的 `env.sh` 不在 `/etc/profile.d` 里，
出网代理只在 Jupyter 进程的环境里。脚本放在 `/root/private_data/scnet_env.sh`
（跟着持久盘走，重启不丢），同时 copy 一份到 `/etc/profile.d/zz-scnet-env.sh`
（跟着镜像走），并在 `/root/.bashrc` 里 source。

代理凭据**优先从正在跑的 `jupyter-lab` 进程动态读**，读不到才用兜底值——
重启后凭据可能变，写死会让脚本悄悄失效。

```bash
. /root/private_data/scnet_env.sh   # 之后 pip / git / wandb / import torch 都正常
```

## 2.7 打包镜像

整套规范与流程（含平台规则、基础镜像地址怎么查、legacy builder 的坑、验收标准）
单独成篇：**[`docs/scnet-image-build.md`](scnet-image-build.md)**。

当前可用镜像：`image.ac.com:5000/dcu/sqrtz/base/anima-lora-dcu:v0.1.1`（运行期自检 7/7 通过）。
用它建实例后，**第一件事**：
