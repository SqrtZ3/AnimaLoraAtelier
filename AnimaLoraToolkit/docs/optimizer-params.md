...
### 10. `adamw_snr` — AdamW + 梯度信噪比门控（cautious / SNR 锐化）

**默认与 `adamw` 数学恒等**（`cautious=false, snr_power=1.0`），单测
`TestDefaultsEquivalentToAdamW` 逐元素对拍断言。两个门控均为逐元素运算，
不含谱平坦化算子（对比 §8 的 muon 结案）。

**关键观察**：AdamW 的更新 `m/sqrt(v)` 逐坐标幅度**就是该坐标的梯度信噪比**，
即 **AdamW ≡ sign(m) × SNR**。所以「按 SNR 抑制噪声方向」这件事 AdamW 本身
已经在做，本模块只是把它做得更锐。

**本仓库实测的 SNR 分布**（Krea2 c12port 成功 run 的 step800 优化器状态，
113.7M 坐标，`r = mu/sigma` 由 `|m|/sqrt(v)` 反解）：

| r 分位 | 25% | 50% | 90% | 95% |
|---|---|---|---|---|
| r | 0（纯噪声）| 0.094 | 0.66 | 0.93 |

- 噪声坐标（r<0.15）占 **54% 的坐标数**，但仅占 **6.84% 的更新能量**
...
optimizer_args:
  ...
  cautious: true        # C-AdamW 掩码（arXiv:2411.16085, ICLR 2025）
  snr_power: 1.0        # >1 开启 SNR 锐化；1.0 = 标准 AdamW
```

**各门控对更新能量分配的预测**（基于上述实测分布计算）：

| 机制 | 信号能量 | 噪声能量 | 信噪预算比 |
|---|---|---|---|
| AdamW β1=0.9（基线） | 66.0% | 6.84% | 9.7 |
| β1=0.98（纯配置改动） | 69.6% | 3.94% | 17.7 |
| `cautious: true` | 79.1% | 2.96% | 26.7 |
| `snr_power: 1.5` | 82.7% | 1.82% | 45.4 |
| `snr_power: 2.0` | 92.2% | 0.42% | 218 |

> ⚠️ **预期管理**：AdamW 已把 93% 的更新预算给了非噪声方向，这些门控争夺的
> 只是剩下 6.8%。且噪声方向随机游走（累积 ∝√t）、信号线性累积（∝t），在最终
> ΔW 里噪声占比更低——与实测 ΔW 极度 rank-1 主导（top1 98%）自洽。
> **画面无明显改善是可能的正常结果**，不要预设它一定有效。

> **另**：`snr_power` 构造期 fail-fast 拦 `<1`（会放大噪声，与目的相反）
> 和 `>4`（>99% 能量集中到 top-17% 坐标，其余近乎冻结）。

**未采纳的候选**：AdEMAMix（ICLR 2025）靠 β3=0.9999 的慢 EMA 挖掘旧梯度，
horizon = 1/(1−β3) = **10000 步**；本仓库典型 run 仅数百至一千步，慢 EMA
全程处于预热期，机制不会启动——**结构性不匹配，未实现**。
