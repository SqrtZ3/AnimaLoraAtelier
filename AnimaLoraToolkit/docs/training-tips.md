...
- 如果 loss 下降太慢 → 提高学习率
- 如果 loss 震荡剧烈 → 降低学习率
- 如果过拟合（采样图变差）→ 回退到更早的 checkpoint，或降低学习率（见下方"过拟合"一节的范式说明）
...
**解决方案**：
1. **回退到更早的 checkpoint**（首选）
2. 降低学习率
3. 增加 tag_dropout（5-15%）
4. 降低 LoRA rank
5. 增加数据多样性

> **训练范式说明**：本项目默认设一个极大 epoch 持续训练、随时手动停止、从任意 step 的 checkpoint 中挑模型。
> 因此"训练时长 / epoch"不是调节旋钮——**过拟合的第一反应是"往回选更早的 checkpoint"，而不是"减少 epoch"**。
> 其余几条（学习率、tag_dropout、rank、数据）是"每一步都生效"的真正可调项。
...
**解决方案**：
1. 继续训练、选更靠后的 checkpoint（同上：靠选 checkpoint 而非加减 epoch）
2. 提高学习率
3. 提高 LoRA rank
4. 检查标签是否正确
5. 检查数据是否正确加载
