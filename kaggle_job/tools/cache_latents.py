...
规划数学（floor 对齐 / 多尺度缩放系数）来自 `_vendor/latent_plan.py` ——
那是从 upstream `trainer/data.py` **逐字摘录**的（见 `_vendor/UPSTREAM.md`），
不在本文件重新实现。单一事实源，避免两处公式漂移。

> 上面提到的 `trainer/*.py:行号` 指的是 upstream 仓库（`anima-lora-train` 的
> `AnimaLoraToolkit/`）里的位置 —— 它们是这些口径的**出处证据**，本仓库不需要
> 那个目录也能跑。要复核到源头就设 `ANIMA_UPSTREAM` 后跑
> `tools/tests/check_cache_parity.py`（判本仓库产物与 upstream 产物逐 bit 相同）。
...
def main() -> int:
    ...
    import torch
    from _vendor.latent_plan import (plan_native_fit_image, plan_multiscale_copy,
                                     tiled_vae_encode)
    from _vendor.st_load import load_vae
    ...
    vae = load_vae(a.vae, dev, torch.bfloat16,
