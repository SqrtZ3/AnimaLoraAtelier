"""TREAD 真模型冒烟：加载 Anima 底模（CPU），验证
1) 手动 per-block 路径与 model() 直通路径的数值一致性（tread 关）
2) TREAD 路由开启时前向可跑、输出有限、形状正确、扰动量级合理

用法:
  python tread_smoke.py --base <anima-base.safetensors> [--dtype fp32|bf16] [--hw 32]
"""
import argparse
import os
import sys
...
REPO = Path(__file__).resolve().parents[1] / "AnimaLoraToolkit"
sys.path.insert(0, str(REPO))

from trainer.models import load_anima_model  # noqa: E402
from trainer.objective import forward_with_optional_checkpoint  # noqa: E402
...
def main():
    ...
    t0 = time.time()
    model = load_anima_model(args.base, "cpu", dtype, REPO / "models")
    ...
    with torch.no_grad():
        ...
        out_tread = forward_with_optional_checkpoint(
            ...
            tread_ratio=args.ratio, tread_start=args.start, tread_end=args.end)
